...
pipeline {
    ...
    stages {
        ...
        stage ("Checkout") {
            steps {
                timestamps {
                    checkout(changelog: false, poll: false, scm: [$class: "GitSCM", branches: [[name: "*/master"]], doGenerateSubmoduleConfigurations: false, extensions: [[$class: "CloneOption", depth: 0, noTags: true, reference: "", shallow: true]], submoduleCfg: [], userRemoteConfigs: [[url: "${env.CLONE_BASE}/annotations.git"]]])
                }
            }
        }
        stage ("Build") {
            ...
            post {
                always {
                    jacoco(execPattern: "**/jacoco.exec", inclusionPattern: "de/shadowhunt/annotation/**/*")
